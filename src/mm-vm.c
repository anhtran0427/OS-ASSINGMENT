...
int __read(struct pcb_t *caller, int rgid, int offset, BYTE *data)
{
  ...
  if(currg == NULL || cur_vma == NULL) /* Invalid memory identify */
	  return -1;
  int rgstart=currg->rg_start;
  if (currg->vmaid==0) rgstart+=1;
  else rgstart-=1;
  pg_getval(caller->mm, rgstart  + offset, data, caller);
...
int __write(struct pcb_t *caller, int rgid, int offset, BYTE value)
{
  ...
  pg_setval(caller->mm, rgstart  + offset, value, caller);
